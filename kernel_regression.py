...
def KernelLogit(x_train, t_train, x_test, folds=5, num_basis=False, sigma_list=None, lda_list=None, algorithm='Ridge'):
            ...
            for lda_idx, lda in enumerate(lda_list):
                if algorithm == 'Lasso':
                    clf = linear_model.LogisticRegression(
                        penalty='l1', C=lda, solver='saga', multi_class='auto')
                elif algorithm == 'Ridge':
                    clf = linear_model.LogisticRegression(
                        penalty='l2', C=lda, solver='saga', multi_class='auto')
                ...
                score_cv[sigma_idx, lda_idx] = score_cv[sigma_idx,
                                                        lda_idx] + score
    ...
    if algorithm == 'Lasso':
        clf = linear_model.LogisticRegression(
            penalty='l1', C=lda_chosen, solver='saga', multi_class='auto')
    elif algorithm == 'Ridge':
        clf = linear_model.LogisticRegression(
            penalty='l2', C=lda_chosen, solver='saga', multi_class='auto')
