...
class RCT(Basic):
    ...
    def effect(self):
        ...
        var = 2*np.mean(y1**2) + 2*np.mean(y0**2) - theta**2
        null_var = 2*np.mean(y1**2) + 2*np.mean(y0**2)
        
        return theta, var, null_var
...
class AdaIPW(Basic):
    ...
    def effect(self):
        ...
        var = np.mean((a*y/p1 - (1-a)*y/p0 - theta)**2)
        null_var = np.mean((a*y/p1 - (1-a)*y/p0)**2)
                                                                                        
        return theta, var, null_var
    
    
class A2IPW(Basic):
    def __init__(self, method, pretraining=50, gamma= lambda t: 1/np.sqrt(t)):
        super().__init__()
        self.func_1 = []
        self.func_0 = []
    ...
    def effect(self):
        ...
        theta = np.mean(z)
        var = np.mean((z - theta)**2)
        null_var = np.mean((z)**2)
                                                                                        
        return theta, var, null_var
    
            
class MA2IPW(Basic):
    def __init__(self, method, pretraining=50, gamma= lambda t: 1/np.sqrt(t), zeta =  lambda t: t**(-1/5)):
        super().__init__()
        self.func_1 = []
        self.func_0 = []
    ...
    def effect(self):
        ...
        var = np.mean((z0 - theta)**2)
        null_var = np.mean((z0)**2)
...
class DM(Basic):
    ...
    def effect(self):        
        ...
        var = np.mean((mu1 - mu0 - theta)**2)
        null_var = np.mean((mu1 - mu0)**2)
...
class Hahn(Basic):
    ...
    def effect(self):        
        ...
        if N > self.first_phase:
            ...
            var2 = np.mean(((z - theta)**2)[self.first_phase:])
            
            pi = self.first_phase/N
            theta = pi*theta + (1-pi)*theta2
            var = pi*var + (1-pi)*var2
            
        null_var = 2*np.mean((y1**2)[:self.first_phase]) + 2*np.mean((y0**2)[:self.first_phase])
        
        if N > self.first_phase:
            null_var2 = np.mean(((z)**2)[self.first_phase:])
            
            pi = self.first_phase/N
            null_var = pi*null_var + (1-pi)*null_var2
